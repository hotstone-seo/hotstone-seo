...
func TestPagination(t *testing.T) {
	...
	tests := []struct {
		...
		{url: "/people?name=foo-name&address=%25foo-address%25", pgnType: NoPagination, sql: "SELECT id, name, address, updated_at FROM people WHERE address LIKE ? AND name = ?", sqlArgs: []interface{}{"%foo-address%", "foo-name"}, param: PaginationParam{Sorts: emptySorts, Filters: map[string]*Filter{"name": &Filter{Col: "name", Cond: "foo-name"}, "address": &Filter{Col: "address", Cond: "%foo-address%"}}, Nexts: emptyNexts}},
