	...
	testcases := []testCase{
		...
		},
		{
			TestCase: echotest.TestCase{
				...
				ExpectedBody: "{\"id\":100,\"name\":\"some-name\",\"path\":\"\",\"pattern\":\"\",\"label\":\"\",\"updated_at\":\"0001-01-01T00:00:00Z\",\"created_at\":\"0001-01-01T00:00:00Z\"}\n",
			},
			moduleCntrlBuilder: moduleCntrlBuilder{
				moduleSvcFn: func(svc *service_mock.MockModuleService) {
		...
		},
		{
			TestCase: echotest.TestCase{
				Request: echotest.Request{
					Method: http.MethodPost,
					Target: "/",
					Body:   `{"id" : 1,"name":"some-name", "path":"some-path","pattern":"string pattern","label":"string label"}`,
					Header: echotest.HeaderForJSON(),
				},
				ExpectedCode: http.StatusOK,
